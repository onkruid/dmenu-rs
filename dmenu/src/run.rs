...
use x11::xlib::{
    ...
    ShiftMask, VisibilityNotify, VisibilityUnobscured, XFilterEvent, XInternAtom, XKeyEvent,
    XLookupBoth, XLookupChars, XLookupKeySym, XNextEvent, XRaiseWindow, XmbLookupString,
};
...
impl Drw {
    pub fn run(&mut self) -> CompResult<()> {
        unsafe {
            ...
            while XNextEvent(self.dpy, ev.as_mut_ptr()) == 0 {
    ...
    fn keypress(&mut self, mut ev: XKeyEvent) -> CompResult<bool> {
        ...
        unsafe {
            let buf: [u8; 32] = [0; 32];
            let mut __ksym = MaybeUninit::uninit();
